...
import {createProductReview} from '../store/actions/product-actions'
import {fetchProductDetails} from '../store/actions/product-detail-actions'
import {productCreateReviewActions} from '../store/reducers/productReducers';
...
function ProductScreen() {
  ...
  const {loading,error,product} = useSelector(state => state.product_detail)
  ...
  useEffect(() =>{
    if(successProductReview){
      setRating(0)
      setComment('')
      dispatch(productCreateReviewActions.PRODUCT_CREATE_REVIEW_RESET())
    }
                          ...
                          <ListGroup variant = 'flush'>
                            {product.reviews.map(review=> (
                              <ListGroup.Item key={review._id}>
                                ...
                                <p>{(review.createdAt).substring(2,10)}</p>
                                <p>{review.comment}</p>
                              </ListGroup.Item>
                            ))}
                            <ListGroup.Item>
